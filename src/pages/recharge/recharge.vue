...
<script>
	...
	methods: {
		...
		cmbPay(orderId) { //招行支付
			...
			http.get($.M.baseUrl, '/pay/CMBPay', param).then((data) => {
		...
		jianhangPay(orderId) { //建行		
			var param = {
				"parameters": {
					"orderId": orderId
				}
			};
			http.get($.M.baseUrl, '/pay/CCBPay', param).then((data) => {
				if(data.hasError == false) {
		...
		ABCPay(orderId) { //农行
			...
			http.get($.M.apiBaseUrl, '/client/v1/pay/ABCPay', param).then((data) => {
				if(data.hasError == false) {
					window.location.href = data.data;
				} else {
					JsTools.msgAlert(data.errorMessage, 1);
		...
		BestPay(orderId) { //翼支付
			...
			http.get($.M.apiBaseUrl, '/client/v1/pay/BestPay', param).then((data) => {
				if(data.hasError == false) {
					window.location.href = data.data;
				} else {
					JsTools.msgAlert(data.errorMessage, 1);
		...
		BocomPay(orderId) { //交行
			...
			http.get($.M.api4BaseUrl, '/client/v1/pay/bocompay', param).then((data) => {
				if(data.hasError == false) {
					$("body").html(data.data);
				} else {
					JsTools.msgAlert(data.errorMessage, 1);
